...
class _TIMUIKitAppBarState extends TIMUIKitState<TIMUIKitAppBar> {
  ...
  Widget tuiBuild(BuildContext context, TUIKitBuildValue value) {
    ...
    return AppBar(
      ...
      elevation: setAppbar?.elevation ?? (isWideScreen ? 0 : 1),
      ...
      centerTitle: setAppbar?.centerTitle ?? (isWideScreen ? false : true),
      leadingWidth: setAppbar?.leadingWidth ?? (isWideScreen ? 8 : 70),
      leading: Selector<TUIChatGlobalModel, Tuple2<bool, int>>(
          builder: (context, data, _) {
            final isMultiSelect = data.item1;
            final unReadCount = data.item2;
            return (!isWideScreen && isMultiSelect)
                ? TextButton(
                    onPressed: () {
